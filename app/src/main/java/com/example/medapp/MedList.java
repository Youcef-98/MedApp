...
public class MedList extends AppCompatActivity implements AdapterMed.OnItemClick {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if( MainActivity.myViewModel.connexion!=true)
        {
        ...
        {
            MainActivity.myViewModel.getWebmedList().observe(this, new Observer<List<Medicament>>() {
                @Override
                public void onChanged(List<Medicament> med) {
                    for(int i=0;i<MainActivity.temp.size();i++)
                    {
                        if(find(MainActivity.temp.get(i).getCodeBarMed(),med)==true)
                        {
                            MainActivity.temp.remove(i);
                            i--;
                        }
                    }
                    med.addAll(MainActivity.temp);
                    ca.setListMed(med);
                }
            });
        }
    ...
    private boolean find(long codeBarMed, List<Medicament> med) {
        for(int i=0;i<med.size();i++)
        {
            if(med.get(i).getCodeBarMed()==codeBarMed)
                return true;
        }
        return false;
    }

...
public class MedViewModel extends AndroidViewModel {
    ...
    private LiveData<List<Medicament>> WebmedList;
    ...
    public MedViewModel(@NonNull Application application) {
        ...
        if(connexion==true)
        {
            ...
            WebmedList=webMedRepository.getMedList();
    ...
    public void delete(Long codeBar,Boolean connexion) {
        if(connexion==true)
            webMedRepository.deleteMedicament(codeBar);
        else
            medRepository.deleteMedicament(codeBar);
    ...
    public LiveData<List<Medicament>> getWebmedList() {
        return WebmedList;
    }
